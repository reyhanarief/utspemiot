...
$query = "
    SELECT 
        ROUND(MAX(suhun), 0) AS suhu_max, 
        ROUND(MIN(suhun), 0) AS suhu_min, 
        ROUND(AVG(suhun), 2) AS suhu_rata
    FROM tb_hydroponic
";
...
if (mysqli_num_rows($result) > 0) {
    ...
    $query_data = "SELECT id, suhun, humid, lux, ts FROM tb_hydroponic";
